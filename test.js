(function () {
  // ==== CONFIG ====
  const INPUT_SELECTOR = 'input.check-room-input'; // 👈 change this to match your target input
  ...
  function findJoinBtn() {
    ...
    for (const b of buttons) {
      ...
      for (const cand of candidates)
        if (new RegExp('\\b' + cand + '\\b', 'i').test(t)) return b;
    }
    return null;
  }
  ...
  panel.innerHTML = `
    ...
    <div style="font-size:11px;opacity:0.8;">Using selector: <code>${INPUT_SELECTOR}</code></div>
  ...
  async function runLoop() {
    ...
    while (!stopped && attempts < MAX_ATTEMPTS && state.pinIndex < allPins.length) {
      ...
      if (!el) { if (!alertShown) { alert(`Input element not found for selector "${INPUT_SELECTOR}".`); alertShown = true; } break; }
